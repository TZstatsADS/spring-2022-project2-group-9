...
vaccination <- read.csv("/Users/xilezhang/OneDrive/GitHub/Spring2021-Project2-group3/app/output/Covid_Vaccine.csv")
vaccination <- as.data.frame(vaccination)

homeless_drop_in <- read.csv("/Users/xilezhang/OneDrive/GitHub/Spring2021-Project2-group3/app/output/Homeless_Drop-In_Centers_location.csv")
homeless_drop_in <- as.data.frame(homeless_drop_in)
...
vaccination <- vaccination %>%
  mutate(category = rep("Vaccination", nrow(vaccination))) %>%
  select(category, Name, Type, Location,Vaccine_offered, Latitude, Longitude) %>%
  filter(!is.na(Latitude) & !is.na(Longitude))


homeless_drop_in <- homeless_drop_in %>%
  mutate(category = rep("Homeless Drop-in Center", nrow(homeless_drop_in))) %>%
  select(category, Name=Center.Name, Location=Address, Comments, Latitude, Longitude) %>%
  filter(!is.na(Latitude) & !is.na(Longitude))
...
data <- full_join(vaccination,homeless_drop_in,by=c("category","Name","Location","Latitude","Longitude"))
